...
async function dfsCycleDetectionTracePath(graphComponentMatrix, srcr, srcc, visited, dfsVisited){
    ...
    await colorPromise(); // waits for 1 sec, afater 1 sec is finished, the code will move further 
    ...
    for(let children = 0;children < graphComponentMatrix[srcr][srcc].length;children++){
        ...
        if(visited[crid][ccid] === false){
            ...
            if(response === true){
                cell.style.backgroundColor = "transparent"; // we are returning so we have to remove the color
                await colorPromise();
